...
Page.prototype.end = function(evt) {
  ...
  console.log(this.isLast, scrolledToBottom, this.animator.animating)
  if(this.prevPage && lastDelta > 0 && scrolledToTop) {
    if(this.prevPage.animator.animating) return
...
Page.prototype.move = function(evt) {
  ...
  if(delta > 0 && scrolledToTop) {
    ...
    if(this.prevPage) {
      ...
      this.prevPage.animator.updatePosition(delta - this.prevPage.clientHeight)
    }
    evt.preventDefault()
  }
  if(delta < 0 && scrolledToBottom) {
    ...
    if(this.prevPage) {
      if(this.prevPage.animator.animating) return evt.preventDefault()
      this.prevPage.updatePosition(delta - this.prevPage.clientHeight)
    }
    evt.preventDefault()
  }
}